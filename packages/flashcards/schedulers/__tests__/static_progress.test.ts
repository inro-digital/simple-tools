...
import StaticProgressScheduler, { type Srs } from '../static_progress.ts'
...
Deno.test('StaticProgressScheduler - init', () => {
  ...
  assert(assignment.unlockedAt, 'should be unlocked')
  assert(assignment.startedAt, 'should be started')
})

Deno.test('StaticProgressScheduler - filter', () => {
  const sched = new StaticProgressScheduler({ srs, userLevel: 2 })
...
Deno.test('StaticProgressScheduler - sort', () => {
  const sched = new StaticProgressScheduler({ srs, userLevel: 2 })
...
Deno.test('StaticProgressScheduler - requiredSubjects', () => {
...
Deno.test('StaticProgressScheduler - update', () => {
  const sched = new StaticProgressScheduler({ srs, userLevel: 2 })
  ...
  assert(success2.passedAt, 'should be passed at efactor 2')
...
Deno.test('StaticProgressScheduler - progress states', () => {
  const sched = new StaticProgressScheduler({ srs, userLevel: 2 })

  let assignment = sched.add(sA)
  assertEquals(assignment.passedAt, undefined, 'not passed initially')
  assertEquals(assignment.completedAt, undefined, 'not completed initially')

  // Progress to passed state (efactor 2)
  assignment = sched.update(true, sA, assignment)
  assignment = sched.update(true, sA, assignment)
  assert(assignment.passedAt, 'passed at efactor 2')
  assertEquals(assignment.completedAt, undefined, 'not completed yet')

  // Progress to completed state (efactor 6)
  for (let i = 0; i < 4; i++) {
    assignment = sched.update(true, sA, assignment)
  }
  assert(assignment.passedAt, 'still passed')
  assert(assignment.completedAt, 'completed at efactor 6')
})
